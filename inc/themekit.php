 ...
 * @return 	void
 */
function igrow_illinois_more_setup() {
...
<!-- Google Tag Manager -->
<noscript><iframe src="//www.googletagmanager.com/ns.html?id=GTM-NTV8B8"
height="0" width="0" style="display:none;visibility:hidden"></iframe></noscript>
<script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':
new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],
j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src=
'//www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);
})(window,document,'script','dataLayer','GTM-NTV8B8');</script>
...
function igrow_illinois_fonts_url() {
	...
	$return 	= '';
	$families 	= '';
	$fonts[] 	= array( 'font' => 'Oxygen', 'weights' => '400,700', 'translate' => esc_html_x( 'on', 'Oxygen font: on or off', 'igrow-illinois' ) );
 ...
 */
function igrow_illinois_get_posts( $post = 'post', $params = array(), $cache = '' ) {

	$return 	= '';
	$prefix 	= 'igrow_illinois_posts_';
	$sep 		= '_';
	...
	$cache_name = $prefix . $post . $cache;

	$return = wp_cache_get( $cache_name, 'igrow_illinois_posts' );
	...
	if ( false === $return ) {
		...
		if ( ! is_wp_error( $query ) && $query->have_posts() ) {
			...
			wp_cache_set( $cache_name, $query, 'igrow_illinois_posts', 5 * MINUTE_IN_SECONDS );
...
function igrow_illinois_get_svg( $svg, $link = '' ) {
	...
	switch ( $svg ) {
		...
		case 'blocks' 			: $output .= '<svg xmlns="http://www.w3.org/2000/svg" viewBox="39.3 40.2 66 66" class="blocks"><path d="M67 72.9c-1.8 2.5-2 5-1.7 7.6.6 5 1.2 9.9 1.9 14.9.5 4.1-.3 6.4-2.4 6.4-3.4 0-3.3-2.9-3.9-5.2-.7-3-1.5-6-2.8-9.1-.4 2.6-.7 5.2-1.1 7.8-.1.9-.1 1.7-.2 2.6-.5 3.1-1.7 4.3-3.7 4.1-2.6-.3-2.7-2.4-2.6-4.3.2-8.9-.9-17.9 1.5-27.5-2.3 1.2-4 2.1-5.8 3-1.1.6-2.1 1.4-3.2 1.6-1 .1-2.2.2-3-1-.8-1.1-.1-2.5.5-3.1 1.7-1.6 3.8-2.9 5.8-4.3 2.4-1.7 5-3.1 7.2-5 3.4-3.1 7.1-3.2 11-1.5 1.5.6 3.1 1.7 3.9 3.1 3 4.8 5.7 9.8 8.5 14.8.9 1.6 1.7 3.3-.4 4.5-2 1.1-3.2-.2-4.3-1.7-1.7-2.8-3.3-5.1-5.2-7.7M59.9 51.1c.1-4 2.9-6.7 7-6.5 3.7.2 6.5 3 6.6 6.6 0 3.6-3.3 6.9-6.9 6.9-3.9 0-6.8-3.1-6.7-7M90.6 83.5H88v-5.2h-2.1v-1.8c1.4 0 2.4-.5 2.7-2.1h2v9.1zm2.3-11.2h-8.1c-1.4 0-2.5 1.1-2.5 2.5v8.1c0 1.4 1.1 2.5 2.5 2.5h8.1c1.4 0 2.5-1.1 2.5-2.5v-8.1c0-1.3-1.1-2.5-2.5-2.5M86.9 96.7v2.2h-7.5c-.1-1 .4-1.8 1.1-2.5 1.5-1.6 3.8-2.2 3.8-3.7 0-.6-.5-.9-1.1-.9-1 0-1.3.8-1.3 1.7h-2.4c0-2.6 1.5-3.7 3.8-3.7 1.9 0 3.6.8 3.6 2.9 0 1.6-1.2 2.2-2.4 2.9-.6.4-1.3.7-1.8 1.1h4.2zm.3-8.8h-8.1c-1.4 0-2.5 1.1-2.5 2.5v8.1c0 1.4 1.1 2.5 2.5 2.5h8.1c1.4 0 2.5-1.1 2.5-2.5v-8.1c.1-1.4-1-2.5-2.5-2.5M98.3 99.1c-2.1 0-3.7-1.1-3.6-3.3h2.5c0 .4.1.7.3 1 .2.2.5.4 1 .4.8 0 1.2-.5 1.2-1 0-1-1-1-1.6-1v-1.6c.6.1 1.6-.1 1.6-.9 0-.6-.5-.8-1-.8-.6 0-1.1.3-1.2 1H95c.1-2.1 1.8-2.9 3.6-2.9 2.1 0 3.4 1 3.4 2.5 0 .8-.3 1.4-1 1.8.8.3 1.3 1 1.3 1.9 0 1.8-1.6 2.9-4 2.9m4.2-11.2h-8.1c-1.4 0-2.5 1.1-2.5 2.5v8.1c0 1.4 1.1 2.5 2.5 2.5h8.1c1.4 0 2.5-1.1 2.5-2.5v-8.1c0-1.4-1.1-2.5-2.5-2.5"/>'; break;
		case 'brain' 			: $output .= '<svg xmlns="http://www.w3.org/2000/svg" viewBox="38.4 41 66 66" class="brain"><path d="M80.1 75.3c2.2-.7 4.3-.9 5.7-2 4.9-3.7 5.2-10.1 1-14.9-6.7-7.7-21.3-7.5-27.7.4-.9 1.1-3.1 2.6-3.5 3.9-.8 2.6 1.8 6.9 4.1 6.8 4.4-.3 7.3 5.2 12.3 2.2.2-.1 1 .5 1.4.9 1.5 1.6 2.9 3.4 4.3 5 .9 1.1 1.8 2.2 2.8 3.3.4-.2.7-.4 1.1-.6-.5-1.7-1.1-3.5-1.5-5m-21.2 29.6s.3-2.4.3-8.2C55.5 96.6 49 96 46.7 93c-2.5-3.2-1-6.3-.9-7.1.3-5-1.4-4.8-2.5-5.6-.9-.7-2.1-3.3-.6-5.1 2.2-2.7 4.1-3.4 4-7.1-2.9-15 8.1-24.4 22.4-26.6 13.2-2 26.7 6.1 30.5 18.1 3.5 10.9.9 20.7-8.2 28.2-2.8 2.3-4.7 5.3-5.4 8-.7 2.7.6 8.9.6 8.9s-7.4 1.9-12.4 2c-5.1.2-15.3-1.8-15.3-1.8"/>'; break;
		case 'family' 			: $output .= '<svg xmlns="http://www.w3.org/2000/svg" viewBox="39.3 39.9 66 66" class="family"><path d="M90.2 55.7s5.7-.8 7.6 6.1c1.4 5.2 7.1 4.9 7.1 4.9s-4.2 5.1-9 0c-5.6-5.9-8.4-3.7-8.4-3.7l2.5-6.9"/><path d="M82.3 79.4l-1.1-.4 9.1-24.6 1.1.4M66.3 86.4c-.3 5.2-.8 11.3-1.2 16.5 0 .5-.8 1.2-1.4 1.3-1 .2-2 .1-3 .1-1.4-.1-1.7-2.4-1.7-2.4s-1.5-10.5-1.4-11.2c.6.3 2 1 2.4 1.4 1.3 1 2.1.1 2.8-1.1l2.7-3.9c.3.1.9-1.5.8-.7M59.2 47.1c-.1 3.2-2.5 5.5-5.7 5.3-3.1-.2-5.3-2.7-5.1-5.9.2-3 2.5-5.1 5.5-5.1 3.3.1 5.4 2.4 5.3 5.7M70.5 55.3c-3.1 0-5.3-2.2-5.2-5.3.1-3 2.5-5.4 5.4-5.2 2.7.1 5 2.5 5.1 5.2 0 3-2.3 5.3-5.3 5.3M73.1 84.8c2.3 1.9 4.8 5.1 4.8 5.1s-.5 8.4-.4 11.5c0 2.2-1.6 2.9-3 2.6-.5-6.4-1-12.5-1.4-19.2M50.5 90.8c0 4 .1 8.7.1 13-2.1-.3-2.8-1.7-2.9-3.4-.1-4.9 0-8.2 0-8.2s1.2-.7 2.8-1.4"/><path d="M85.5 74.5C83 70 80.6 65.4 78 61c-3.1-5.3-9.3-6.2-14.2-1.9-1.4-5.1-5.1-6-9.7-5.6-1.3.1-2.7 0-4 0-3.4.2-5.3 1.6-6 4.8-1.5 6.6-2.8 13.3-4.3 19.9-.4 1.7 0 3.1 1.8 3.4 1.7.4 2.3-1 2.7-2.3.9-3 1.8-6 2.7-8.9.2 0 .4.1.6.1v18.7c2.1-1.5 2.8-4 5.1-4.8-1-.6-1.8-1.7-1.8-3 0-1.9 1.5-3.4 3.3-3.4 1.8 0 3.3 1.5 3.3 3.4 0 1.4-.8 2.5-2 3.1 2.1.9 3.1 3.5 5.2 5 .8-2.1 2-4 2.7-5.7 1-2.1 2.3-3 4.5-3.7-1.2-.6-2.1-1.9-2.1-3.3 0-2 1.6-3.7 3.7-3.7 2 0 3.7 1.7 3.7 3.7 0 1.5-.9 2.8-2.2 3.4 1.8.3 2.8.9 4.7 3.2 1.3 1.4 2.3 3.4 2.7 3.3.4-.1 5.6.1 5.5-.3 0-1-4-7.3-4-7.3s-1.6-3.9-2.2-7.2c1.7 1.5 2.8 3.2 4 4.9.8 1.2 1.9 2.1 3.3 1.2 1.8-.9 1.2-2.3.5-3.5"/>'; break;
		case 'handandheart' 	: $output .= '<svg xmlns="http://www.w3.org/2000/svg" viewBox="38.1 40.1 66 66" class="handandheart"><path d="M69.6 77.8c7.9.6 16.4 1.9 22.7-1.6 1.8-1 3.8-2.1 5.7-2.1 1.7 0 4.3-.2 5 1.6.8 2.1-1.6 3.1-2.8 3.9C85 89.3 71.8 90.4 54.5 89.9c-1.1 0-4 .2-4.1-2.1-.1-2.2-.6-10.2.7-14.9 1.2-4.2 2.5-4.1 7.1-3.8 4.4.3 6.4 1.4 10.8.7 2.9-.5 8.9-4.8 10.6-.9 1.4 3.2-1.5 4.7-4.2 6-1.2.5-3.6 1.6-5.8 2.9M38.4 95.1c-.1-7.3-.3-14-.2-20.7 0-4 .4-5.7 4.6-5.1 1.8.3 4.9.3 7.4.4-2.4 7-2 12.6-1.2 20.6.1.9-2.1 2.7-3.7 3.1-1.6.3-2.7 1-6.9 1.7M96.6 53.6c-2.5.4-4.1 1.7-5 2.8-.5-1.3-1.5-3.2-3.8-4.4-3.9-2-9.6-.4-9.8 6.8-.1 5.4 9.8 15.6 10.6 15.3.5.4 13.2-5.8 14.9-10.9 2.3-6.8-2.5-10.2-6.9-9.6"/>'; break;
		case 'house' 			: $output .= '<svg xmlns="http://www.w3.org/2000/svg" viewBox="39.2 40.1 66 66" class="house"><path d="M103.2 62.6L72.1 42.4 40.9 62.6c-2 2-2 5.3 0 7.3h4.2V98c0 3.1 2.5 5.7 5.6 5.7H65c-.1-.3-.2-.6-.2-.9v-17c0-1.2 1-2.2 2.2-2.2h10.1c1.2 0 2.2 1 2.2 2.2v16.9c0 .3-.1.6-.2.9h14.2c3.1 0 5.6-2.5 5.6-5.7v-28h4.2c2.1-2 2.1-5.3.1-7.3"/>'; break;
		case 'pram' 			: $output .= '<svg xmlns="http://www.w3.org/2000/svg" viewBox="39.5 40.7 66 66" class="pram"><path d="M50.3 73.3c1.5-.7 2.6-2.2 2.6-4 0-2.4-1.9-4.3-4.3-4.3s-4.3 1.9-4.3 4.3c0 1.8 1.1 3.3 2.6 4-1.8.8-3 2.7-3 4.9 0 2.9 2.1 5.3 4.7 5.3s4.7-2.4 4.7-5.3c0-2.2-1.3-4.1-3-4.9"/><path d="M65.6 96.4c-1.3 0-2.3-1-2.3-2.3 0-1.3 1-2.3 2.3-2.3 1.3 0 2.3 1 2.3 2.3 0 1.3-1 2.3-2.3 2.3m-18.2 0c-1.3 0-2.3-1-2.3-2.3 0-1.3 1-2.3 2.3-2.3s2.3 1 2.3 2.3c0 1.3-1 2.3-2.3 2.3m31.8-28.5c-1.4.9-5.9 8.3-5.9 8.3s.3-5-4.6-6.2c-4.9-1.3-9.1-2.8-9.3-3.2s-3.3-4.1-3.3.8c0 4.9.6 8.8-2.8 8.8-2.6 0-7.4.1-7.4.1s-3.8-.5-5.4.2c-1.6.7-.6 3.4-.2 4.6.8 2.4 2.8 6.1 4.7 7.7-1.9.9-3.2 2.8-3.2 5.1 0 3.1 2.5 5.6 5.6 5.6 3.1 0 5.6-2.5 5.6-5.6 0-1.2-.4-2.3-1-3.2 2.7.3 5.9.5 8.7.5-.4.8-.7 1.7-.7 2.7 0 3.1 2.5 5.6 5.6 5.6 3.1 0 5.6-2.5 5.6-5.6 0-1.9-1-3.7-2.5-4.7 3.4-3.9 10.4-15.3 11.7-17.4 1.5-2.4 4.3-1.3 4.3-1.3v-3.5s-4.1-.3-5.5.7M104.8 48.9c-1.1-2-3.2-2.9-4.7-2-.6.3-1 .8-1.2 1.5-1.1-.6-2.3-1-3.6-1-4 0-7.2 3.2-7.2 7.2s3.2 7.2 7.2 7.2 7.2-3.2 7.2-7.2v-.3c.6.1 1.2 0 1.7-.3 1.4-.8 1.7-3.1.6-5.1M91 66.2c-2.1 1.5-3.2 2.2-3.4 3.5-.3 2 1.4 3.3 1.4 3.3s.1.6-.8 1.3c-10.9 2.2-7.4 17.2-7.4 17.2l10.3.4 2.1 8.8 4.1.1V63.3h-3.8s-.4 1.5-2.5 2.9"/>'; break;
 ...
 * @return 	mixed 					The SVG code
 */
function igrow_illinois_the_svg( $svg, $link = '' ) {

	echo igrow_illinois_get_svg( $svg, $link = '' );
...
/**
 * Adds PDFs and vCards as a filters for the Media Library
 *
...
/**
 * Allows for uploading vCards to the Media Library
 *
 * @param 	array 		$existing_mimes 		The current MIME types
 * @return 	array 								The modified MIME types
 */
 ...
 */
function igrow_illinois_excerpt_length( $length ) {
